  ...
  return (
    <div className="min-h-screen bg-[#0A192F] p-6 lg:p-10 text-white">
      <header className="max-w-7xl mx-auto flex justify-between items-center border-b border-white/5 pb-6 mb-8">
        <div>
          <div className="text-[#64ffda] text-xs font-bold tracking-wider uppercase">📋 PLC COLLABORATION HUB</div>
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="bg-[#172A45] border border-white/5 rounded-xl p-6 shadow-xl space-y-6">
            <div className="flex justify-between items-center border-b border-white/5 pb-4">
              <h2 className="text-xl font-bold text-white">PLC Meetings</h2>
              {isTeacherOrLeader && meetings.length > 0 && (
                <button
                  onClick={() => router.push(`/dashboard/plc/run/${meetings[0].id}`)}
                  className="bg-[#64ffda] text-[#0A192F] hover:bg-[#52e0c2] font-bold py-2 px-4 rounded-md text-sm transition-colors cursor-pointer"
                >
                  🚀 Run Active Meeting
                </button>
              )}
            ...
            {meetings.length === 0 ? (
              ...
              <div className="space-y-4">
                {meetings.map(meeting => (
                  <div 
                    key={meeting.id} 
                    className="bg-[#0A192F]/40 border border-white/5 rounded-lg p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-white/10 transition-colors"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[#64ffda] text-xs font-bold uppercase tracking-wider">{meeting.team.name}</span>
                        <span className="text-white/30 text-xs">•</span>
                        <span className="text-[#8892B0] text-xs">{new Date(meeting.date).toLocaleDateString()}</span>
                      </div>
                      <h3 className="font-bold text-white text-base leading-snug">{meeting.objective}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="bg-white/5 border border-white/10 text-white/80 text-[10px] font-bold px-2 py-0.5 rounded">
                          Standard: {meeting.standard}
                        </span>
                        <span className="text-xs text-[#8892B0] truncate max-w-xs">{meeting.learning_target}</span>
                      </div>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          {profile?.role === 'teacher' && (
            <div className="bg-[#172A45] border border-white/5 rounded-xl p-6 shadow-xl space-y-4">
              <h3 className="text-base font-bold text-white border-b border-white/5 pb-3">My Assigned Actions</h3>
              {actionItems.length === 0 ? (
                <div className="text-sm text-[#8892B0] py-4">You have no active action items assigned.</div>
              ) : (
                <div className="space-y-3">
                  {actionItems.map(item => (
                    <div 
                      key={item.id} 
                      className="flex items-start gap-3 bg-[#0A192F]/40 p-3 rounded-lg border border-white/5"
                    >
                      <input 
                        type="checkbox"
                      ...
                      <div className="space-y-1 text-xs">
                        <p className={`font-semibold text-white/95 leading-normal ${
                          item.status === 'Completed' ? 'line-through text-white/40' : ''
                        }`}>
                          {item.action}
                        </p>
                        <div className="flex justify-between items-center text-[10px] text-[#8892B0] pt-1">
          ...
          {/* Team Members info */}
          <div className="bg-[#172A45] border border-white/5 rounded-xl p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-white border-b border-white/5 pb-3">My PLC Teams</h3>
            {teams.length === 0 ? (
              <div className="text-sm text-[#8892B0] py-4">You are not currently enrolled in any PLC teams.</div>
            ) : (
              <div className="space-y-2">
                {teams.map(t => (
                  <div key={t.id} className="flex justify-between items-center bg-[#0A192F]/40 p-3 rounded-lg border border-white/5">
                    <span className="font-semibold text-white text-sm">{t.name}</span>
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
                      t.is_leader ? 'bg-[#64ffda]/10 text-[#64ffda]' : 'bg-white/5 text-[#8892B0]'
                    }`}>
