...
export default function PlcMeetingWizard({ params }: { params: Promise<PageParams> }) {
  ...
  return (
    <div className="min-h-screen bg-[#0A192F] p-6 lg:p-10 text-white">
      <header className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-6 mb-8 gap-4">
        <div>
          <div className="text-[#64ffda] text-xs font-bold tracking-wider uppercase">📝 MEETING WIZARD</div>
          <h1 className="text-3xl font-extrabold tracking-tight mt-1">PLC Collaboration Cycle</h1>
          <p className="text-[#8892B0] text-sm mt-1">{meeting?.team.name} • Facilitator: {profile?.first_name} {profile?.last_name}</p>
        </div>
        <button
          onClick={async () => {
            await handleSaveStep();
            ...
            <div key={index} className="flex-1 text-center relative">
              <div className="flex items-center justify-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-all ${
                  step === index + 1
                    ? 'border-[#64ffda] bg-[#64ffda]/10 text-[#64ffda] shadow-[0_0_10px_rgba(100,255,218,0.2)]'
                    : step > index + 1
                    ? 'border-emerald-500 bg-emerald-500 text-[#0A192F]'
                    : 'border-white/10 bg-[#172A45]/30 text-[#8892B0]'
                }`}>
                  {step > index + 1 ? '✓' : index + 1}
                </div>
              </div>
              <span className={`block text-xs font-semibold mt-2 ${
                step === index + 1 ? 'text-[#64ffda]' : 'text-[#8892B0]'
              }`}>
                {stepName}
              </span>
            </div>
      ...
      <main className="max-w-4xl mx-auto bg-[#172A45] border border-white/5 rounded-xl p-8 shadow-xl space-y-6">
        
        {/* STEP 1: Set the Goal */}
        {step === 1 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 1: Set the Goal</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8892B0] mb-2">Meeting Objective</label>
                <textarea 
                  value={objective}
                  onChange={(e) => setObjective(e.target.value)}
                  placeholder="e.g. Plan support targets and analyze student work artifacts..."
                  className="w-full h-20 bg-[#0A192F]/60 border border-white/10 rounded-md p-3 text-white text-sm focus:border-[#64ffda] focus:outline-none resize-none"
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-[#8892B0] mb-2">North Carolina Standard</label>
                  <select 
                    value={standard}
        ...
        {step === 2 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 2: Review Student Evidence</h2>
            <p className="text-xs text-[#8892B0]">Roster evidence collected on the selected standard and core competency levels:</p>
            <div className="bg-[#0A192F]/40 border border-white/5 rounded-lg p-5 text-center text-[#8892B0]">
        ...
        {step === 3 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 3: Group Students by Instructional Need</h2>
            <p className="text-xs text-[#8892B0] mb-4">Dynamically place students into strategic, skill-specific groups. These groups are temporary and target-specific.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              
              {/* Students Pool (4 cols) */}
              <div className="md:col-span-4 bg-[#0A192F]/40 border border-white/5 rounded-lg p-4 space-y-3">
                <h4 className="text-xs font-bold uppercase tracking-wider text-[#64ffda] border-b border-white/5 pb-2">Student Pool</h4>
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                  {students.map(s => (
                    <div key={s.id} className="bg-[#172A45] p-2.5 rounded border border-white/5 text-xs">
                      <span className="font-bold text-white block">{s.first_name} {s.last_name}</span>
                      <select 
                        onChange={(e) => moveStudentToGroup(s.id, e.target.value)}
                        className="w-full mt-2 bg-[#0A192F]/60 border border-white/10 rounded py-1 px-2 text-white text-[10px] focus:outline-none"
                        defaultValue=""
                      >
              ...
              <div className="md:col-span-8 space-y-3">
                {Object.keys(studentGroups).map(groupName => (
                  <div key={groupName} className="bg-[#0A192F]/20 border border-white/5 rounded-lg p-3 space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-[#64ffda]">{groupName}</span>
                      <span className="bg-white/5 text-[10px] px-2 py-0.5 rounded text-[#8892B0]">
                        {studentGroups[groupName].length} assigned
                      </span>
                    </div>
                    {studentGroups[groupName].length === 0 ? (
                      <div className="text-[10px] text-[#8892B0] italic p-2 border border-dashed border-white/5 rounded">No students assigned.</div>
                    ) : (
                      <div className="flex flex-wrap gap-1.5">
                        {studentGroups[groupName].map(s => (
                          <span key={s.id} className="bg-[#172A45] border border-white/10 text-white text-[10px] px-2.5 py-1 rounded-full font-medium">
        ...
        {step === 4 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 4: Analyze Root Causes</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8892B0] mb-2">What did students learn & what evidence supports this?</label>
                <textarea 
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8892B0] mb-2">Are there visible root causes / misconceptions? (Instruction, Tasks, Attendance)</label>
                <textarea 
                  value={rootCauses}
        ...
        {step === 5 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 5: Assign Actions</h2>
            
            {/* List Action items */}
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#8892B0]">Current Action Items Roster</h4>
              {actions.length === 0 ? (
                <div className="text-sm text-[#8892B0] bg-[#0A192F]/20 p-4 rounded-md border border-dashed border-white/5">
                  No action items assigned yet. Use the form below to add team duties.
                </div>
              ) : (
                <div className="space-y-2">
                  {actions.map((act, index) => (
                    <div key={index} className="bg-[#0A192F]/40 border border-white/5 p-3 rounded-lg flex justify-between items-center text-xs">
                      <div>
                        <p className="font-semibold text-white">{act.action}</p>
                        <span className="text-[10px] text-[#8892B0]">Owner: {profile?.first_name} {profile?.last_name} • Deadline: {new Date(act.deadline).toLocaleDateString()}</span>
                      </div>
            ...
            <form onSubmit={handleAddAction} className="bg-[#0A192F]/30 p-4 rounded-lg border border-white/5 space-y-4">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#64ffda]">➕ Add Action Item</h4>
              <div className="space-y-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-[#8892B0] mb-1.5">Action Strategy</label>
                  <input 
                    type="text"
                    value={newActionText}
                    onChange={(e) => setNewActionText(e.target.value)}
                    placeholder="e.g. Deploy ELA cause-effect exit ticket scaffolding frames..."
                    className="w-full bg-[#0A192F]/60 border border-white/10 rounded py-2 px-3 text-white text-xs focus:border-[#64ffda] focus:outline-none"
                    required
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-[#8892B0] mb-1.5">Deadline</label>
                    <input 
                      type="date"
              ...
              <button 
                type="submit"
                className="w-full bg-[#64ffda]/10 border border-[#64ffda]/20 text-[#64ffda] hover:bg-[#64ffda]/20 py-2 rounded font-bold text-xs transition-colors cursor-pointer"
              >
        ...
        {step === 6 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-white border-b border-white/5 pb-3">Step 6: Review & Close</h2>
            <div className="space-y-4">
              <p className="text-sm text-[#8892B0]">
                All data, goals, groupings, and action items have been drafted. Clicking complete locks this cycle.
              </p>
              <div className="bg-[#0A192F]/40 border border-white/5 rounded-lg p-5 space-y-2 text-xs">
                <p><span className="text-[#64ffda] font-bold uppercase">Standard:</span> {standard}</p>
                <p><span className="text-[#64ffda] font-bold uppercase">Objective:</span> {objective}</p>
                <p><span className="text-[#64ffda] font-bold uppercase">Action Plan items count:</span> {actions.length}</p>
              </div>
              <button 
                onClick={handleCompleteMeeting}
                className="w-full bg-emerald-500 text-[#0A192F] hover:bg-emerald-400 font-extrabold py-3 rounded-md text-sm transition-colors shadow-lg cursor-pointer"
              >
